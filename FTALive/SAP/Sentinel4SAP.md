...
<!--
Sentinel at Scale の 「概要」

SIEM (脅威の分析と対応) と SOAR (対応の自動化)

データコネクタ でログを収集

分析ルールが脅威を検出する。分析すべき対象は Watchlist という特別なテーブルでカスタマイズすることができる。
アナリストとオペレーターの分離を可能にして、監視対象や注目するイベントはオペレーターに任せることができる。

ワークブックでセキュリティ状態を把握する
...
## Microsoft Sentinel のアーキテクチャ （ドキュメントを準備、別で分離しても良いかも）
...
3. これらのトランザクションコードは、Sentinel Workspace上の Watchlist `SAP - Sensitive Tables` で管理されています。
